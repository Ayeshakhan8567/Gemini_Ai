...
const Sidebar = () => {
  ...
  const { 
    onSent, 
    allChats, // Changed to read global sessions list instead of current chat array
    loadPastChat, // Added to reload full history sessions when clicked
    setRecentPrompt, 
    newChat, 
    setResultData, 
    setShowResult,
...
border
border-white/10
shadow-[0_0_60px_rgba(168,85,247,0.35)] flex flex-col h-screen p-3 transition-all duration-300 justify-between ${
        extended ? "w-70" : "w-20"
      }`}
    >
      {/* Top Content Wrapper */}
      <div className="w-full flex flex-col gap-5">
        ...
        {/* Header Section (Gemini Logo & Menu) */}
        {!extended ? (
          ...
          <div className="flex items-center justify-between animate-fade-in">
            ...
            {/* Menu Icon */}
            <div className="flex w-9 h-9 items-center justify-center rounded-full hover:bg-white/40 cursor-pointer transition-all duration-200 hover:scale-105 active:scale-95">
              <img
                src={assets.menu_icon}
                alt="Menu"
        ...
        {/* Navigation Actions */}
        <div className={`second_div flex flex-col gap-3 transition-all duration-300 ${extended ? "w-full" : "w-full items-center"}`}>
          ...
          <div onClick={() => newChat()}
            className={`flex flex-row gap-2 h-10 transition-all duration-200 hover:bg-white/40 hover:rounded-2xl cursor-pointer items-center hover:scale-[1.02] active:scale-95 ${
              extended ? "w-full pl-2" : "w-10 justify-center rounded-xl"
            }`}
          >
            <img
          ...
          >
            ...
            />
            {extended && <p className="font-medium text-sm text-gray-800">Images</p>}
          </div>
          ...
          >
            <img
              ...
              className="w-8 h-8 object-contain"
            />
            {extended && <p className="font-medium text-sm text-gray-800">Library</p>}
          </div>

        </div>

        {/* Recent Prompts History Session List */}
        {extended && allChats && allChats.length > 0 && (
          <div className="recent flex flex-col gap-2 transition-all duration-300 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="max-h-60 overflow-y-auto flex flex-col gap-1 pr-1 custom-scrollbar">
              {allChats.map((item, index) => (
                <div
                  key={index}
                  onClick={() => loadPastChat(item.id)} // Loads the complete past session matching this ID
      ...
      <div className={`bottom flex items-center justify-between pt-4 border-t border-black/10 ${extended ? "flex-row w-full" : "flex-col gap-4 w-full"}`}>
